...
function CompoundConditionalCalculator() {
    ...
    const handleCalculateIF = () => {
        ...
        axios.post('http://localhost:8080/calculate-complexity-if', { codeIF })
            .then(response => {
    ...
    const handleCalculateFor = () => {
        ...
        axios.post('http://localhost:8080/calculate-complexity-for', { codeFor })
            .then(response => {
                setComplexityFor(response.data);
            })
            .catch(error => {
                setErrorFor('Error calculating FOR complexity. Please check your input and try again.');
        ...
        <div className='container'>
            ...
            <div className='content'>
                <div className='CCMTcontainer'>
                    <button className='btncal' onClick={() => window.location.href = '/'}>Home</button>
                </div>
                <h1 style={{ color: "white" }}>Compound Conditional Statements Complexity Calculator</h1>
                <div style={{ display: 'flex' }}>
                    <div style={{ marginRight: '60px' }}>
                        ...
                        {complexity !== null && <p>Calculated IF Complexity: {complexity}</p>}
                    </div>
